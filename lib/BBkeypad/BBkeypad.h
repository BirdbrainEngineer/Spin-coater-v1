...
enum KeyState {
    ...
    PRESSED,    // Key is pressed and is waiting to transition into 'HELD' state after the debounce interval has elapsed
    HELD,       // Key is currently being held
};
...
class BBkeypad {
    ...
    public:
        ...
        bool poll(); // Polls the keypad for key press changes. Returns 'true' if a key's state has changed.
